...
class FactorRepositoryTest {
  ...
  fun `Get all factors with a list of not only strings should filter`() {
    ...
    val factorValues = listOf(factorToJson1, factorToJson2, intValue)
    ...
    val factors = factorRepository.getAll()
    assertNotEquals(factorValues.size, factors.size)
    assertEquals(factorValues.filterIsInstance<String>().size, factors.size)
    assertTrue(factors.contains(factor1))
    assertTrue(factors.contains(factor2))
  }
