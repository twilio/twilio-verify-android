...
internal class DefaultSerializer : Serializer {
  ...
  override fun <T : Any> fromByteArray(
    ...
    when {
      ...
      kClass.isAssignableFrom(Int::class) ->
        String(
          data,
        ).toInt() as? T
      kClass.isAssignableFrom(Double::class) ->
        String(
          data,
        ).toDouble() as? T
      ...
      kClass.isAssignableFrom(Float::class) ->
        String(
          data,
        ).toFloat() as? T
      kClass.isAssignableFrom(Long::class) ->
        String(
          data,
        ).toLong() as? T
      else -> throw IllegalArgumentException("Value type not supported")
...
internal fun <T : Any, R : Any> KClass<T>.isAssignableFrom(kClass: KClass<R>): Boolean = this.javaObjectType.isAssignableFrom(kClass.javaObjectType)
